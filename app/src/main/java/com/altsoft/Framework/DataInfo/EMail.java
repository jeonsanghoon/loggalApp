...
public class EMail extends javax.mail.Authenticator {
    ...
    public EMail() {
        ...
        _port = "465"; // default smtp port
        _sport = "465"; // default socketfactory port
        _user = "mrc0700smtp@gmail.com"; // username
        _pass = "jshgogoman3"; // password
    ...
    public boolean send() throws Exception {
        ...
        if (!_user.equals("") && !_pass.equals("") && _to.length > 0
            ...
            if(Global.getValidityCheck().isEmpty(_fromName))
            ...
            if(!Global.getValidityCheck().isEmptyStringArray(_toCc)) {
                /*참조*/
                InternetAddress[] addressToCc = new InternetAddress[_toCc.length];
                for (int i = 0; i < _toCc.length; i++) {
                    addressToCc[i] = new InternetAddress(_toCc[i]);
                }
                msg.setRecipients(MimeMessage.RecipientType.CC, addressToCc);
            }
            if(!Global.getValidityCheck().isEmptyStringArray(_toCc)) {
                /*숨은참조*/
                InternetAddress[] addressToBcc = new InternetAddress[_toBcc.length];
                   for (int i = 0; i < _toBcc.length; i++) {
                    addressToBcc[i] = new InternetAddress(_toBcc[i]);
                }
                msg.setRecipients(MimeMessage.RecipientType.BCC, addressToBcc);
            }
            ...
            if(_isHtml)
            {
                ...
                htmlPart.setContent(_body, "text/html;charset=utf-8;");
    ...
    public void set_isHtml(boolean _isHtml) {
